...
int main (int argc, char *argv [])
{
  ...
  if (!in)
  {
    printf (" Couldn't reopen the CBF file %s\n", argv [2]);

    exit (1);
  }

  cbf_failnez (cbf_read_file (cbf, in))
  ...
  printf (" Time to read the CBF image: %.3fs\n", ((b - a) * 1.0) / CLOCKS_PER_SEC);
  ...
  {
    printf (" The dimensions of the CBF image don't match the original\n");
  ...
  for (column = 0; column < img_columns (cbf_img); column++)
    ...
    for (row = 0; row < img_rows (cbf_img); row++)
      ...
      {
        printf (" The CBF image differs from the original at (%d, %d)\n", column, row);

        exit (1);
      }

  printf (" The CBF image matches the original\n");
